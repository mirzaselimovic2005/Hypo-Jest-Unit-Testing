...
const InputForm = ({ onCalculate }) => {
  ...
  return (
    <div className="input-form">
      <label>
        Eigen Inkomen:
        <input type="number" value={inkomen} onChange={(e) => setinkomen(e.target.value)} />
      </label>
      <label>
        Partner Inkomen:
        <input type="number" value={inkomen2} onChange={(e) => setinkomen2(e.target.value)} />
      </label>
      <label>
        Rentevaste periode:
        <select value={renteperiode} onChange={(e) => setrenteperiode(e.target.value)}>
      ...
      <label>
        ...
        <input type="checkbox" checked={studentenschuld} onChange={(e) => setstudentenschuld(e.target.checked)} />
      </label>
      <label>
        Postcode:
        <input type="number" value={postcode} onChange={(e) => setPostcode(e.target.value)} />
      </label>
      <button onClick={calcHypotheek}>berekenen</button>
    </div>
  );
}
